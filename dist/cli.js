...
export function parseArgs(argv) {
    ...
    if (command !== 'search')
        throw new HeroldError(`Unbekannter Befehl: ${command}`, 2);
    const kind = normalizeKind(subcommand);
    if (!kind)
        throw new HeroldError('Erwartet: herold search firms|people ...', 2);
    const flags = readFlags(rest);
    const term = kind === 'firms' ? (flags.what ?? flags.name) : flags.name;
    if (!term)
        throw new HeroldError(kind === 'firms' ? 'Firmensuche braucht --what <branche|firma> oder --name <firma>.' : 'Personensuche braucht --name <name>.', 2);
    const query = {
        kind,
        term,
    ...
    if (kind === 'firms')
        query.mode = flags.name ? 'name' : 'what';
    if (flags.where)
        query.where = flags.where;
    if (flags.limit)
        query.limit = positiveInt(flags.limit, '--limit');
    return { query };
}
function normalizeKind(value) {
    if (value === 'firms' || value === 'firm' || value === 'firma' || value === 'firmen')
        return 'firms';
    if (value === 'people' || value === 'person' || value === 'personen')
        return 'people';
    return null;
}
function readFlags(args) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (!arg.startsWith('--'))
            throw new HeroldError(`Unerwartetes Argument: ${arg}`, 2);
        const name = arg.slice(2);
        const value = args[index + 1];
        if (!value || value.startsWith('--'))
            throw new HeroldError(`Flag --${name} braucht einen Wert.`, 2);
        flags[name] = value;
        index += 1;
    }
    return flags;
...
function positiveInt(value, flag) {
    ...
    if (!Number.isInteger(parsed) || parsed < 1)
        throw new HeroldError(`${flag} muss eine positive Ganzzahl sein.`, 2);
    return parsed;
}
function renderResponse(response, stdout) {
