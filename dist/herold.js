...
export function buildSearchUrl(query) {
    ...
    if (query.kind === 'firms') {
        if (!query.where)
            throw new HeroldError('Firmensuche braucht --where <ort>.', 2);
        if (query.mode === 'name') {
...
export async function searchHerold(query, fetchImpl = fetch) {
...
async function searchFirmByName(query, fetchImpl) {
    ...
    for (const suggestion of filtered.slice(0, 8)) {
        ...
        if (!response.ok)
            continue;
        const parsed = parseSearchResults(await response.text(), 'firms');
...
async function fetchFirmNameSuggestions(term, fetchImpl) {
    ...
    if (!Number.isInteger(entryIndex) || entryIndex < 0 || entryIndex >= data._objs.length)
        return [];
    ...
    return resolved
        ...
        .filter((option) => isRecord(option) && typeof option.id === 'string' && typeof option.value === 'string' && isRecord(option.extraOptions));
}
function resolveQwikValue(objs, value, seen = new Set()) {
...
function matchesWhere(extraOptions, where) {
    if (!where)
        return true;
...
function rankFirmResults(results, where) {
    if (!where)
        return results;
    const normalizedWhere = normalizeText(where);
    return [...results].sort((left, right) => whereScore(right, normalizedWhere) - whereScore(left, normalizedWhere));
}
function whereScore(result, normalizedWhere) {
    const city = normalizeText(result.city ?? '');
    const address = normalizeText(result.address ?? '');
    if (city === normalizedWhere)
        return 3;
    if (new RegExp(`\\b${escapeRegExp(normalizedWhere)}\\b`, 'i').test(address))
        return 2;
    if (city.includes(normalizedWhere))
        return 1;
    return 0;
}
...
function parsePersonResults(data) {
    ...
    for (const raw of data.objs) {
        if (!isRecord(raw))
            continue;
        if (!('pid' in raw) || !('cryptedPid' in raw) || !('displayName' in raw) || !('mainNumber' in raw))
            continue;
...
function parseContacts(data, rawContacts) {
    if (!Array.isArray(rawContacts))
        return [];
    return rawContacts.map((contact) => parseContact(data, contact)).filter((contact) => contact !== null);
}
function parseContact(data, rawContact) {
...
function resolveRef(data, value) {
    if (typeof value !== 'string')
        return value;
    ...
    if (!Number.isInteger(index) || index < 0 || index >= data.objs.length)
        return value;
    return data.objs[index];
}
function materialize(data, value, depth = 0) {
    if (depth > 4)
        return value;
    const resolved = resolveRef(data, value);
    if (Array.isArray(resolved))
        return resolved.map((item) => materialize(data, item, depth + 1));
...
//# sourceMappingURL=herold.js.map